/**************************************************************************
 ...
 ------------------------------------------------------------------------
 DESCRIPTION:
    This software manages 4 servos and the polarization from for each
    swtich. 
    Configuration is done through SV Loconet protocol and can be configured
    from Rocrail (Programming->GCA->GCA50).
    Ports from 1 to 4 define the output number for each servo. Type of
    output must be "Switch".
    Ports from 8 to 11 define the input number for the feedback signal.
 ------------------------------------------------------------------------
 PIN ASSIGNMENT:
   0,1 -> Serial, used to debug and Loconet Monitor (uncomment DEBUG)
   2,3,4,5 -> Attach one servo motor to each pin   
   7 -> Loconet TX (connected to GCA185 shield)
   8 -> Loconet RX (connected to GCA185 shield)
   9,10,11,12 -> Attach relay for frog polarization
 ------------------------------------------------------------------------
 SHIELDS NEEDED:
  * Loconet Shield: ViDa LocoShield or GCA185
  * 4 relay expansion board
  * Sensor Shield v5 for easy connection of relay and servo
 ------------------------------------------------------------------------
 CREDITS: 
...
void setup()
{
  ...
  pinMode(PIN_RELAY+1, OUTPUT);
  pinMode(PIN_RELAY+2, OUTPUT);
  pinMode(PIN_RELAY+3, OUTPUT);
  pinMode(13, OUTPUT);
...
void moveServo(int pNumServo, int pDestPos)
{
  ...
  else
  {
    ...
    #ifdef DEBUG    
    Serial.print("MOVING SERVO ");Serial.println(pNumServo+1);
    Serial.print("From     : ");Serial.println(pDestPos);
    Serial.print("MidGrades: ");Serial.println(midgrades);
    Serial.print("To       : ");Serial.println(servoCurrentPos[pNumServo]);
    #endif 
    for (grades=servoCurrentPos[pNumServo];grades>=pDestPos;grades--)
    {
      positionServo(pNumServo,grades);
